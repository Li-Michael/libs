...
def parse_strandbam(fbamfile,chrom,start,end,sslib=3,mapq=30):
	"""
	R: the single read is in the antisense (reverse) orientation
	F: the single read is in the sense (forward) orientation
	RF: first read (/1) of fragment pair is sequenced as anti-sense (reverse(R)), and second read (/2) is in the sense strand (forward(F)); typical of the dUTP/UDG sequencing method. (dUTP)
	...
	if sslib >2:spe = 1 #双端数据
	if sslib == 1:
		it = fbamfile.fetch(chrom,start,end)
		for read in it:
			if read.mapq < mapq or read.is_unmapped or read.is_secondary:continue
			read1blocks =  read.get_blocks()
			if read.is_reverse:
				strand = "+"
			else:strand = "-"
	...
	#count = 0
		...
		idxdict = {(True,True):"1+",(False,False):"2-",(True,False):"1-",(False,True):"2+"}
		"PE 数据必须2个read 都有mapping"
		it = fbamfile.fetch(chrom,start,end)
		for read in it:
			if read.mapq < mapq or  read.is_unmapped or read.is_secondary or read.mate_is_unmapped:continue
			if not read.is_paired:continue
			strand = idxdict[(read.is_read1,read.is_reverse)] # strand  same as RNA
			readblocks =  read.get_blocks()
			yield [readblocks,None,None,strand]
	elif sslib == 4:
		it = fbamfile.fetch(chrom,start,end)
		for read in it:
			if read.mapq < mapq or read.is_unmapped or read.is_secondary or read.mate_is_unmapped:continue
			...
			if read.is_read1:
				if read.reference_id <> read.next_reference_id:
					sys.stderr.write("[WARN] PE on different chromsome\n%s\n%s\n"%(fbamfile.references[read.reference_id],fbamfile.references[read.next_reference_id]))
				read2 = fbamfile.mate(read)
				read1blocks =  read.get_blocks()
				read2blocks = read2.get_blocks()
				if read.is_reverse:
					strand = "-"
