...
def group_scatter(xs,ys,groups,xlabel,ylabel,addline=None,fig_prefix="test",alpha=0.6,colors=None):
	if colors == None:
	...
	for i in xrange(len(groups)):
		...
		x = xs[i]
		y = ys[i]
		patch = ax.scatter(x,y,marker=markers[i],linewidths=0,color=colors[i],alpha=alpha)
		print patch
		patchs.append(patch)
		if addline != None:
...
def plot_boxplotscatter(X,fig_prefix,xlabel,ylabel,xticks_labels,colors=None,ylim=[],scatter=1,markersize=7):
	fig = plt.figure(dpi=300)
	ax = fig.add_subplot(111)
	bp = ax.boxplot(X)
	for box in bp['boxes']:
	...
	if scatter:
		for i in xrange(len(X)):
			...
			ax.plot(x, X[i], 'o',color="#e7298a" ,alpha=0.3,markersize=markersize)
...
def plot_dfboxplot(df,fig_prefix,xlabel,ylabel,outshow=False,colors=None,ylim=[],markersize=6,showmeans=False,showscatter=False):
	...
	if colors == None: colors = styles(nlegends)[0]
	xpos = np.arange(nxticklabels)
	width = 0.9/nlegends
	if showscatter: alpha=0.2 
	else:alpha=1.0
	fig = plt.figure(dpi=300)
	ax = fig.add_subplot(111)
	slplot = []
	for i in xrange(nlegends):
		...
		for j in xrange(nxticklabels):
			if showscatter:
				...
				ax.plot(ttx, tx, 'o',color=colors[i] ,alpha=0.3,markersize=markersize)
			box = bp['boxes'][j]
			boxX = box.get_xdata().tolist(); boxY = box.get_ydata().tolist(); boxCoords = zip(boxX,boxY);
			boxPolygon = Polygon(boxCoords, facecolor=colors[i],alpha=alpha)
			ax.add_patch(boxPolygon)
		sp, = ax.plot([1,1],'o',color=colors[i])
		slplot.append(sp)
	"""
	ax.legend(loc='upper center', bbox_to_anchor=(0.5, 1.1),ncol=6,borderaxespad=0, fancybox=True)
	fig.tight_layout(rect = [0,0,1,0.9])
	"""
	ax.legend(slplot,uniq_legends,loc='upper center', bbox_to_anchor=(0.5, 1.1),ncol=6,borderaxespad=0, fancybox=True)
	for sp in slplot: sp.set_visible(False)
	ax.set_xticks(xpos+width/2*(nlegends-1))	
	ax.set_xticklabels(uniq_xticklabels,rotation=45)	
	ax.set_xlabel(xlabel)
	ax.set_ylabel(ylabel)
	ax.set_xlim(0-width*1.4/2,xpos[-1]+1-width/2)
	if ylim:ax.set_ylim(-10,10)
	ax.grid(False)
	ax.grid(True,axis='y')
	fig.tight_layout(rect = [0,0,1,0.9])
...
def err_line_group(data,error,group_label,xticklabel,xlabel,ylabel,colors,fig_prefix,title=None,xlim=None,ylim=None):
	num_groups,p = data.shape
	assert num_groups == len(group_label)
	fig = plt.figure(dpi=300)
	ax = fig.add_subplot(111)
