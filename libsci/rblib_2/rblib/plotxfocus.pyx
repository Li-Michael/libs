...
def framey(ax):
	...
	for l in ax.get_xticklines() + ax.get_yticklines():
		l.set_markersize(0)
def mutation_spec(snnum,targetgenelist,dfgroup,groups,mutlist,genemutdf,fig_prefix="testplot",width=0.94,CNV=None,cnvlist=None): # input shold first to sort
	...
	cnvcolors = ["#0076AF","#C0C0C0","#E31915"]
	...
	for i in xrange(ngenes-1,-1,-1):
		...
		if 1:
			...
			for j in xrange(len(cnvlist)):
				...
				if tmpidx.sum() >= 1:
					rects = stackvAX.bar(xlocations[tmpidx.values],ones[tmpidx.values]*0.9,width,color=cnvcolors[j],linewidth=0,alpha=0.6,bottom=cumtmp[tmpidx.values],align='edge')
