...
def nancorrmatrix_self_mp2(data,method=0,nprocessor=8,prefix="test"):
...
def nancorrmatrix_self(data,method=0):
	corr = stats.pearsonr if method else stats.spearmanr
	sys.stderr.write("[INFO] use %s method\n"%["pearson","spearmanr"][bool(method)])
	n,p = data.shape
	results = []
	for i in xrange(p-1):
...
if __name__ == "__main__":
	a = np.asmatrix(np.random.random((9,5)))
	b = np.asmatrix(np.random.random((9,3)))
	corr_mat,prob_mat =  corr_matrix(a,b)
	print corr_mat.shape
	print corr_mat
	print matrix_1Dnarr(corr_mat)
