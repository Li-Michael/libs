...
int get_permutation(char *vipname,char *pername,long c_demensionx_adv,long c_ntimes,double *pvalue_adv)
{
	...
	demensionx_adv = (dimen_t) c_demensionx_adv;
	//demensionx_hyb = 151438;

	demensiony = 1;
	dimen_t ntimes = (dimen_t) c_ntimes;
	dimen_t i,k;
	//dimen_t j;
	...
	if ((fp_beta=fopen(vipname,"rb"))==NULL){
		err_open();
		return 1;
	}
	if ((fp_per=fopen(pername,"rb"))==NULL){
		err_open();return 1;
	}
	while(!feof(fp_beta))
	...
	for(k=0;k<ntimes;k++)
	{
		for(i=0;i<demensiony;i++)
		{
			...
			cmpvector(pvalue_adv,beta_ind_adv[i]->vector,per_adv[i],demensionx_adv);
	...
	for(i=0;i<demensiony;i++)
	{
		recover_pvalue(pvalue_adv, beta_ind_adv[i]->index, beta_ind_adv[i]->num);
	...
	for(i=0;i<demensiony;i++)
	{
		get_pvalue(pvalue_adv, demensionx_adv, ntimes);
